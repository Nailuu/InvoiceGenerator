...
public class JasperService {
    ...
    public void generateInvoice() throws JRException, IOException, InvocationTargetException, IllegalAccessException {
        ...
        parameters.put("TOTAL_HOURS", Timestamp.getTimestampFromMinutes(this._taskService.getAll().stream().mapToInt(Task::getMinutes).sum()));

        List<Method> getters = JasperUtils.getGetterMethods(this._templateProperties);
        for (Method getter : getters) {
            String name = getter.getName().substring(3).toUpperCase();
